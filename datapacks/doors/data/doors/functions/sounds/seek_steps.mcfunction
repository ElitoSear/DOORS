...
execute unless entity @e[tag=seek,scores={seekrun=1..}] run stopsound @s * custom:entity.seek.steps_far
...
execute if entity @e[tag=seek,scores={seekrun=1..}] unless entity @s[scores={killedbyseek=1..}] if entity @s[scores={seekstepsfar=1}] unless entity @e[tag=seek,scores={stop=1..}] run playsound custom:entity.seek.steps_far master @s
